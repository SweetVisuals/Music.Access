...
const EmptyStateCard: React.FC<EmptyStateCardProps> = ({ icon: Icon, title, description, height = "h-auto md:h-[350px]" }) => {
    return (
        <div className={`w-full ${height} rounded-xl border border-dashed border-white/10 bg-neutral-900/20 flex flex-col items-center justify-center text-center p-6 animate-in fade-in duration-500`}>
            ...
            <p className="text-neutral-500 text-[11px] md:text-sm max-w-[280px] md:max-w-xs leading-relaxed">{description}</p>
        </div>
    );
};
