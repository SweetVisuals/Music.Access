...
const MobileCart: React.FC<MobileCartProps> = ({ onNavigate, projects = [] }) => {
    ...
    return createPortal(
        ...
        `}>
                ...
                <div className="flex items-center gap-3">
                    {showCheckout ? (
                        <button
                            ...
                            className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center text-white border border-white/10 hover:bg-white/10"
                        >
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-mono text-neutral-500 uppercase">
                            {showCheckout ? 'Secure Payment' : `${items.length} ${items.length === 1 ? 'Item' : 'Items'}`}
                        </p>
                    </div>
                </div>
                <button
            ...
            {/* Content Area */}
            <div className={`flex-1 overflow-y-auto scroll-smooth custom-scrollbar relative ${showCheckout ? 'bg-black' : ''}`}>
                {showCheckout ? (
            ...
            {/* Footer / Summary (Only show if NOT in checkout) */}
