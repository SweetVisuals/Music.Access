...
const MusicPlayer: React.FC<MusicPlayerProps> = ({
    ...
    const { audioRef, currentTime, duration, isScrubbing, setCurrentTime, setDuration, setIsScrubbing, seek } = usePlayer();
            ...
            if (playPromise !== undefined) {
                playPromise.catch(error => {
                    if (error.name === 'AbortError') {
                        // Expected when skipping tracks rapidly
                        // console.log('Playback aborted due to track switch');
                    } else {
    ...
    // Force minimize on track change if autoMinimize is enabled (e.g. Feed autoflow)
    useEffect(() => {
        if (autoMinimize) {
            setIsMinimized(true);
        }
    }, [currentTrackId, autoMinimize]);
