...
const StageWizard: React.FC<StageWizardProps> = ({ config, initialData, onClose, onSave }) => {
    ...
    const renderField = (field: StageField, value: any, onChange: (val: any) => void, pathPrefix: string = '') => {
                                ...
                                {field.fields?.map(subField => (
                                    <div key={subField.id} className="space-y-2">
                                         <div className="flex items-center justify-between">
                                            ...
                                            {subField.aiEnabled && (
                                                <button
                                                    onClick={() => openAiHelper(field.id, idx, subField.id)}
                                                    className="flex items-center gap-1 text-[10px] text-primary hover:underline hover:text-primary/80"
                                                >
                            ...
                            {field.options.map(option => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`px - 4 py - 2 rounded - lg text - xs font - bold border transition - all relative overflow - hidden ${
    isPri
                        ...
                        {field.allowCustom && (
                             <div className="relative group/input">
                                 <input
                                    ...
                                    className={`w - full bg - neutral - 900 border border - neutral - 800 rounded - xl px - 4 py - 3 pr - 12 text - white focus: border - primary / 50 focus: outline - none transition - all ${
    (field.allowSecondary
        ...
        : ''
                                 ...
                                 />
                                  {field.aiEnabled && !activeAiTarget && (
                                    <button
                                        onClick={() => openAiHelper(field.id)} // This needs context if inside array... 
                                        // Wait, openAiHelper (top level) only works for top level fields. 
                                        // For nested, we handled it in the array map above.
                                        // But this input is generically rendered.
                                        // Since we pass 'openAiHelper' down? No.
                                        // Simpler: The caller of renderField handles the AI button for simple types?
                                        // Or we pass the click handler?
                                        // We'll rely on the top loop for Top Level fields AI buttons.
                                        // And the Array Loop for Nested Level fields AI buttons.
                                        // So we DON'T render AI button inside renderField to avoid context hell for now.
                                        // We can conditionally render if we pass a prop.
                                        className="hidden" // Hiding here, handled by wrapper
                                    />
                                )}
                             </div>
                        )}
                                    ...
                                    <div key={field.id} className="space-y-3 bg-neutral-900/20 p-6 rounded-xl border border-neutral-800/50 hover:border-neutral-700 transition-colors">
                                        ...
                                        <div className="flex items-center justify-between mb-2">
                                            <div className="flex flex-col">
                                             ...
                                             {/* Top Level AI Button */}
                                             {field.aiEnabled && field.type !== 'array' && (
                                                <button
                                        ...
                                        {renderField(field, formData[field.id], (val) => updateField(field.id, val))}
                                    </div>
                                ))}
                            </div>
                        </div>
